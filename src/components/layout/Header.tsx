...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-20 bg-[#09090e]/80 backdrop-blur-xl border-b border-white/5 px-4 md:px-8 py-4 flex items-center justify-between gap-4">
      {/* Mobile Brand / Greeting */}
      <div className="flex items-center gap-3">
        <div className="md:hidden flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-pink-500 to-purple-500 flex items-center justify-center shadow-md">
            <Sparkles className="w-4 h-4 text-white" />
          </div>
          <span className="font-bold text-white tracking-wider text-sm">{siteConfig.appName}</span>
        </div>
      ...
      <div className="flex items-center gap-3">
        <Link
          href="/karaoke"
          className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-gradient-to-r from-pink-500/20 to-purple-500/20 border border-pink-500/30 text-xs font-semibold text-pink-300 hover:from-pink-500/30 hover:to-purple-500/30 transition-all shadow-sm"
        >
          <Mic2 className="w-3.5 h-3.5 text-pink-400 animate-pulse" />
          <span className="hidden sm:inline">Karaoke Studio</span>
        </Link>

        <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-purple-600 to-pink-600 p-[1px] shadow-sm">
