...
import IncomingCall from "@/components/together/IncomingCall";
import ActiveVoiceCall from "@/components/together/ActiveVoiceCall";
...
export default function VoiceCallProvider() {
  ...
  // Handle browser tab/window unload to notify partner and clean up WebRTC
  useEffect(() => {
    const handleUnload = () => {
      const store = useVoiceCallStore.getState();
      if (store.callStatus !== "idle") {
        cleanupCall(false, "Tab closed");
      }
    };
      ...
      <AnimatePresence>
        {callError && (
          <motion.div
            ...
            className="fixed top-5 left-1/2 -translate-x-1/2 z-[60] w-[90%] max-w-md p-4 rounded-2xl bg-gradient-to-r from-rose-950/95 via-rose-900/90 to-red-950/95 border border-rose-500/40 backdrop-blur-xl shadow-2xl text-white shadow-rose-950/40 flex items-start justify-between gap-3 select-none"
          >
